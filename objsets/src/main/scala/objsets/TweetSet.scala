...
abstract class TweetSet {
   ...
   */
    def mostRetweeted: Tweet = {
      var maxRet : Option[Tweet] = None
      foreach(t => {
        if (maxRet.isEmpty)
          maxRet = Some(t)
        else if (maxRet.get.retweets < t.retweets)
          maxRet = Some(t)
      })
      if (maxRet.isEmpty) throw new java.util.NoSuchElementException()
      else maxRet.get
    }
   ...
   */
    def descendingByRetweet: TweetList = {
      try {
        lazy val tweet = mostRetweeted
        lazy val nextSet = remove(tweet)
        new Cons(tweet, nextSet.descendingByRetweet)
      } catch {
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet) = {
    var s:TweetSet = new Empty
    foreach(t => s = s.incl(t))
    that.foreach(t => s = s.incl(t))
    s
  }
